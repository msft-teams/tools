...
// const cssFunction = (_name: any, cssProperties: any) => {
...
var upn: string | undefined = '';
let accessToken: string;
let idToken: string;


/**
 * State for the DigitalBadge React component
 */
...
/**
 * Properties for the DigitalBadge React component
 */
...
/**
 * Implementation of the Digital Badge content page
 */
export default class DigitalBadge extends TeamsBaseComponent<IDigitalBadgeProps, IDigitalBadgeState> {
              ...
              response.json().then(datada => {
                ...
                this.props.context.spHttpClient.get(getsitedata, SPHttpClient.configurations.v1).then((user: SPHttpClientResponse) => {
                  user.json().then((dataiduser: any) => {
                    ...
                    let dataexists = datada.value.find((x: any) => x[siteconfig.CName + "Id"] === id);
                    if (dataexists) {
                      ...
                      if (usernamearray.length === 1) {
                        ...
                        userassignletters = usernamearray[0][0].toUpperCase() + usernamearray[usernamearray.length - 1][0].toUpperCase();
                      }
                      this.setState({ showAccept: true, userletters: userassignletters });
                    }
    ...
    this.props.context.spHttpClient.get(getListUrl, SPHttpClient.configurations.v1).then((response: SPHttpClientResponse) => {
      if (response.status === 200) {
        ...
        if (response.status === 404) {
          this.props.context.spHttpClient.get(this.state.siteurl + "/_api/SP.UserProfiles.PeopleManager/GetMyProperties", SPHttpClient.configurations.v1).then((responseuser: SPHttpClientResponse) => {
            responseuser.json().then((datauser: any) => {
              const createsiteurl: string = this.state.siteurl + "/_api/SPSiteManager/create";
              ...
              this.props.context.spHttpClient.post(createsiteurl, SPHttpClient.configurations.v1, spHttpsiteClientOptions).then((siteresponse: SPHttpClientResponse) => {
                if (siteresponse.status === 200) {
                  siteresponse.json().then((sitedata: any) => {
                    if(sitedata.SiteId){
                    var list = {
                      "displayName": siteconfig.list,
                      "columns": [
                        {
                          "name": siteconfig.CName,
                          "personOrGroup": {}
                        }
                      ],
                      "list": {
                    ...
                    this.props.context.msGraphClientFactory
                      .getClient().then((client: MSGraphClient) => {
                        client
                          .api("sites/" + sitedata.SiteId + "/lists")
                          .version("v1.0").header("Content-Type", 'application/json').responseType('json').post(list, (err, _res, rawresponse) => {
                            if (!err) {
                              ...
                              if (rawresponse.status === 201) {
                                ...
                                this.props.context.spHttpClient.get(getsitedata, SPHttpClient.configurations.v1).then((user: SPHttpClientResponse) => {
                                  user.json().then((data: any) => {
                                    ...
                                    listDefinition[siteconfig.CName + "Id"] = id;
      ...
      <div>
        {
          ...
          <Spinner size={SpinnerSize.large} ariaLabel={strings.LoadingSpinnerLabel} label={strings.LoadingSpinnerLabel} ariaLive='assertive' />
        }
        {
          !this.state.isLoading &&
          <TeamsComponentContext
            ...
            <ConnectedComponent render={(props: { context: any; }) => {
              const { context } = props;
                          ...
                          {!this.state.isLoading &&
                            <section aria-live='polite' className={'contentSection'}>
                              {!this.state.hasAccepted &&
                                <div>
                                  <img src={require('../assets/images/appbanner648.jpg')} className={'bannerimage'} alt={strings.BannerImageAlt} />
                                  {this.state.badgeImgURL}
                                  <h1 className={'title'}>{strings.PreAcceptPageTitle}</h1>
                                </div>
                              }
                              {this.state.hasAccepted &&
                                <h1 className={'title'}>{strings.PageTitle}</h1>
                              }
                              {!this.state.hasAccepted && this.state.showAccept &&
                                <p className={'description'} dangerouslySetInnerHTML={this.createMarkup(strings.PreAcceptDisclaimer)} />
                              }
                              {!this.state.hasAccepted && !this.state.showAccept &&
                                <p className={'description'} dangerouslySetInnerHTML={this.createMarkup(strings.NotQualifiedPreAcceptDisclaimer)} />
                              }
                              {this.state.hasAccepted && !this.state.isApplied && this.state.profileImage.url && this.state.profileImage.url !== '../assets/images/noimage.png' &&
                                <p className={`description`} dangerouslySetInnerHTML={this.createMarkup(strings.PreApplyDisclaimer, anchorClass)} />
                              }
                              {this.state.hasAccepted && !this.state.isApplied && this.state.profileImage.url &&
                                <p className={`description`} dangerouslySetInnerHTML={this.createMarkup(strings.PreApplyDisclaimer1, anchorClass)} />
                              }

                              <div className={'profileContainer'}>
                                {this.state.profileImage.url && this.state.hasAccepted && this.state.profileImage.url !== '../assets/images/noimage.png' &&
                                  <div id='forDomToImage' style={{ 'maxWidth': '700px' }}>
                                    <img style={{ width: `${this.state.profileImage.width}px` }} src={this.state.profileImage.url} id={'profileImage'} alt={strings.ProfileImageAlt} />
                                    <img style={{ width: `${this.state.profileImage.width}px`, marginTop: `-${this.state.profileImage.width}px` }} id={'badgeImage'} alt={strings.BadgeImageAlt} src={require('../assets/images/badge648.png')} />
                                  </div>
                                }
                                {this.state.profileImage.url && this.state.profileImage.url === '../assets/images/noimage.png' && this.state.hasAccepted &&
                                  <div id='forDomToImage' style={{ 'maxWidth': '700px' }}>
                                    <img src={require('../assets/images/noimage.png')} style={{ width: `100px` }} id={'profileImage'} alt={strings.ProfileImageAlt} />
                                    <div className={'profiletext'}>{this.state.userletters}</div>
                                    <img style={{ width: `100px`, marginTop: `-100px` }} id={'badgeImage'} alt={strings.BadgeImageAlt} src={require('../assets/images/badge648.png')} />
                                  </div>
                                }
                                {!this.state.profileImage.url && this.state.hasAccepted &&
                                  <div>
                                    <img src={require('../assets/images/noprofile.png')} id={"photoStuff"} alt={'strings.NoProfileImageAlt'} aria-hidden='true' />
                                  </div>
                                }
                              </div>
                              {!this.state.profileImage.url && this.state.hasAccepted &&
                                <p className={'description'} dangerouslySetInnerHTML={this.createMarkup(strings.NoProfileImageDescription, anchorClass)} />
                              }
                                ...
                                <div className={'buttonContainer'}>
                                  {!this.state.hasAccepted && this.state.showAccept &&
                                    <PrimaryButton
                                      ...
                                      ariaDescription={strings.AcceptButtonAriaDescription}
                                    />
                                  }
                                  {!this.state.hasAccepted && !this.state.showAccept &&
                                    <p className={'description'} style={{ 'color': 'red' }} dangerouslySetInnerHTML={this.createMarkup(strings.UnauthorizedText)} />
                                  }
                                  {this.state.profileImage.url && this.state.hasAccepted && !this.state.isApplying && !this.state.isApplied &&
                                    <div className={'buttonContainer'}>
                                      <PrimaryButton
                                        className={primaryButton(contextCSS)}
                                        ...
                                        ariaDescription={strings.ApplyButtonAriaDescription}
                                        disabled={this.state.isApplying || this.state.isApplied || this.state.error.length > 0}
                                      />
                                      <br />
                                      {this.state.profileImage.url !== '../assets/images/noimage.png' && <CompoundButton
                                        iconProps={{ iconName: 'Download' }}
                                        className={compoundButton(contextCSS).container}
                                        style={styleProps}
                                        onClick={this._onDownloadImage}
                                        ariaLabel={strings.DownloadButtonText}
                                        ariaDescription={strings.DownloadButtonAriaDescription}
                                        disabled={this.state.isApplying || this.state.isApplied || this.state.imageDownloaded}
                                        secondaryText={this.state.imageDownloaded ? strings.DownloadedButtonSecondaryText : strings.DownloadButtonSecondaryText}>
                              ...
                              {this.state.isApplying && !this.state.isApplied &&
                                <div className={'applySpinnerContainer'}>
                                  <Spinner ariaLabel={strings.ApplySpinnerLabel} size={SpinnerSize.large} label={strings.ApplySpinnerLabel} ariaLive='assertive' />
                                </div>
                              }
                              {this.state.isApplied && !this.state.isApplying &&
                                <div className={'messagingContainer'}>
                                  <MessageBar ariaLabel={strings.SuccessMessage} messageBarType={MessageBarType.success}>
                                    <span dangerouslySetInnerHTML={this.createMarkup(strings.SuccessMessage, anchorClass)} />
                                  </MessageBar>
                                </div>
                              }
                              {this.state.error &&
                                <div className={'messagingContainer'}>
                                  <MessageBar ariaLabel={this.state.error} messageBarType={MessageBarType.error}>{this.state.error}</MessageBar>
                                </div>
                              }
                            </section>
                          }
                          <canvas id='profileCanvas' width={this.state.profileImage.width} height={this.state.profileImage.width} style={{ display: 'none' }}></canvas>
                          <canvas id='profileCanvasDownload' width={this.state.profileImage.width} height={this.state.profileImage.width} style={{ display: 'none' }}></canvas>
  ...
  public getPhotoBits(): Promise<any> {
    ...
    } else { // other browsers ** this isn't currently working **
      console.log("Function msToBlob not found. Using custom function.");
      return this._getCanvasBlob(canvas).then((blob: any) => {
        return blob;
      }).catch((err: string) => {
        console.error('getPhotoBits error: ' + err);
        this.setState(
          {
            error: 'error',
            isApplying: false,
  ...
  private _onRenderCanvas(profileImage: IProfileImage): Promise<any> {
    let promise: Promise<any> = new Promise<any>((resolve: any, _reject: any) => {
      ...
      profileImageObj.onload = () => {
        context.drawImage(profileImageObj, 0, 0, `${profileImage.width}`, `${profileImage.width}`);
        contextDownload.drawImage(profileImageObj, 0, 0, `${profileImage.width}`, `${profileImage.width}`);
        context.drawImage(badgeImageObj, 0, 0, `${profileImage.width}`, `${profileImage.width}`);
      };
  ...
  private _onRenderCanvasNoImage(profileImage: IProfileImage): Promise<any> {
    let promise: Promise<any> = new Promise<any>((resolve: any, _reject: any) => {
      ...
      profileImageObj.onload = () => {
        ...
        context.fillText(this.state.userletters, canvas.width / 2, canvas.height / 2);
  ...
  private _onApplyProfileImage() {
    ...
    this.getPhotoBits().then((res: any) => {
      this.updateUserPhoto(res).then((response: { status: number; }) => {
        ...
        console.error('_updateUserPhoto error: ' + err);
        this.setState(
          {
            error: strings.ErrorMessage,
            isApplying: false,
      ...
      console.error('_updatePhoto error: ' + err);
    });
  }

  private _getCanvasBlob(canvas: { toBlob: (arg0: (blob: any) => void) => void; }) {
  ...
  private _onDownloadImage() {
    this.setState({ imageDownloaded: true, downloadText: strings.DownloadingButtonText });
    let canvasDownload: any = document.getElementById('profileCanvasDownload');
    let link: HTMLAnchorElement = document.createElement('a');

    if (canvasDownload.msToBlob) { // for IE
  ...
  public updateUserPhoto(blob: any): Promise<any> {
    let photoPromise: Promise<any> = new Promise((resolve: (arg0: Response) => void, _reject: any) => {
      ...
      this.props.context.msGraphClientFactory
        .getClient().then((client: MSGraphClient) => {
          client
            .api("me/photo/$value")
            .version("v1.0").header("Content-Type", 'image/jpeg').responseType('json').put(blob, (err, _res, rawresponse) => {
              if (!err) {
                resolve(rawresponse);
              }
            });
  ...
  public getgraphMyPhotoBitsUrl(): Promise<any> {
    let photoPromise: Promise<any> = new Promise((resolve: (arg0: Response) => void, reject: any) => {
      this.props.context.msGraphClientFactory.getClient().then((garphClient: MSGraphClient) => {
        garphClient.api(graphMyPhotoBitsUrl).version("v1.0").headers({ "Content-Type": 'blob', "responseType": 'blob' })
          .responseType('blob').get().then(data => { resolve(data); })
          .catch(err => {
            reject(err);
          });
      });
  ...
  public getgraphMyPhotoApiUrl(): Promise<any> {
    let photoPromise: Promise<any> = new Promise((resolve: (arg0: Response) => void, _reject: any) => {
      this.props.context.msGraphClientFactory.getClient().then((garphClient: MSGraphClient) => {
        garphClient.api(graphMyPhotoApiUrl).version("v1.0").headers({ "Content-Type": 'blob', "responseType": 'blob' }).responseType('json').get().then(data => { resolve(data); });
      });
    });
    return photoPromise;
  }
  ...
  public showUserInformation(_upn1: string) {
    ...
    this.getgraphMyPhotoBitsUrl().then(blob => {
