...
class ClbAddMember extends React.Component<IClbAddMemberProps, IState> {
                        ...
                        this.props.context.spHttpClient.get(this.state.siteUrl + "/_api/web/lists/GetByTitle('Member List')/fields/GetByInternalNameOrTitle('FocusArea')", SPHttpClient.configurations.v1)
                            ...
                            .then((response: SPHttpClientResponse) => {
                                response.json().then(focusAreas => {
                                    if (!focusAreas.error) {
                                        this.setState({ groups: groups.Choices, focusAreas: focusAreas.Choices });
                                    }
                                });
                            });
                    }
                ...
                if (response.status === 200) {
                    ...
                    await response.json().then((responseJSON: any) => {
                        ...
                        while (i < responseJSON.value.length) {
                            if (responseJSON.value[i] && responseJSON.value[i].hasOwnProperty('Title')) {
    ...
    public async _createorupdateItem() {
        return this.props.context.spHttpClient.get(this.state.siteUrl + "/_api/SP.UserProfiles.PeopleManager/GetMyProperties", SPHttpClient.configurations.v1).then((responseuser: SPHttpClientResponse) => {
                    ...
                    this.props.context.spHttpClient.get(this.state.siteUrl + "/_api/web/lists/GetByTitle('Member List')/Items", SPHttpClient.configurations.v1)
                        .then((responsen: SPHttpClientResponse) => {
                            responsen.json().then(datada => {
                                let memberDataId = datada.value.find((d: { Title: string; }) => d.Title.toLowerCase() === datauser.Email.toLowerCase());
                                let memberidData = memberDataId !== undefined ? memberDataId.Role.toLowerCase() : 'User';
                                this.setState({ memberrole: memberidData });
                                if (this.state.UserDetails.length > 0) {
                                    let email = this.state.UserDetails[0].ID.split('|')[2];
                                    // tslint:disable-next-line: no-shadowed-variable
                                    this.props.context.spHttpClient.get("/_api/web/siteusers?$filter=UserPrincipalName eq '" + email + "'", SPHttpClient.configurations.v1).then((responseuser: SPHttpClientResponse) => {
                                        if (responseuser.status === 200) {
                                            responseuser.json().then(async data => {
                                                const listDefinition: any = {
                                                    ...
                                                    "Role": 'Champion',
                                                    "Status": this.state.memberrole === "manager" || this.state.memberrole === "Manager" || this.state.memberrole === "MANAGER"
                                                        || localStorage["UserRole"] === "Manager" ? 'Approved' : 'Un Approved',
                                                    "Group": this.state.memberData.group,
                                                    "FocusArea": (this.state.memberData.focusArea || 'Teamwork')
                                                };
                                                    ...
                                                    this.props.context.spHttpClient.post(url, SPHttpClient.configurations.v1, spHttpClientOptions)
                                                        .then((response: SPHttpClientResponse) => {
                                                            if (response.status === 201) {
                                                                this.setState({ UserDetails: [], isAddChampion: false });
                                            ...
                                            alert("Response status " + responseuser.status + " - " + responseuser.statusText);
                                        }
                                    });
                                }
                            });
            ...
            <div className={styles.clbHome}>
                <div className="container">
                    ...
                    <div style={{ float: "right", marginTop: "1rem" }}>
                        <button className="btn btn-success mr-2" onClick={() => this._createorupdateItem()}>Save</button>
                        <button className="btn btn-secondary" onClick={() => this.props.onClickCancel()}>Cancel</button>
                    </div>
                    <br></br>
                    <br></br>
